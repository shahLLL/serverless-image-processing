...
def test_config_requires_expected_environment_variables(monkeypatch):
    # Confirms the app raises a clear error when required environment
...
def test_copy_image_to_destination_calls_s3_copy(monkeypatch):
    # Verifies the function builds the destination key correctly and calls the
    # S3 copy operation with the expected bucket and key.
...
def test_store_metadata_uses_dynamodb_table(monkeypatch):
    # Confirms metadata is written to the expected table with the expected
    # fields.
    config = lambda_function.Config()
    stored_items = []
...
def test_send_notification_publishes_to_sns(monkeypatch):
    # Verifies the notification message is published to the configured SNS
    # topic.
    config = lambda_function.Config()
    published = []
...
def test_process_single_record_returns_false_on_failure(monkeypatch):
    # Confirms a single record returns failure when one of the processing
    # steps raises an exception.
    config = lambda_function.Config()
...
def test_lambda_handler_returns_207_when_any_record_fails(monkeypatch):
    # Verifies the handler returns a partial-success response when some
    # records fail and others succeed.
    monkeypatch.setattr(
        lambda_function,
        "Config",
